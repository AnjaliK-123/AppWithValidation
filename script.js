document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (event) {
        ...
        if (!(/^\S.*\S$/.test(name) && name.length >= 3)) {
            showError('name', 'Name must be at least 3 characters and cannot start or end with whitespace');
            valid = false;
        }
        ...
        if (password === '' || password.length < 8) {
            showError('password', 'Password must not be empty and should be at least 8 characters');
            valid = false;
        }
